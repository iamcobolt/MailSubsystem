...
The planner is backlog-driven. A sync pass queues analysis when unanalyzed mail exists, and core keeps draining the analysis backlog before scheduling embedding, location, or filing side work. Location queues filing preview only when there are pending filing recommendations. Core claim priority keeps analysis ahead of older side-work rows and keeps pipeline work ahead of heartbeat and sub-agent support work, so Mail Assistant background tasks cannot block sync, analysis, location, or filing progress. This keeps an idle core from repeatedly spending compute on empty analysis/location passes or printing the same filing preview over and over.
...
Core queue backpressure is fail-fast and bounded by active rows (`pending`, `failed`, or `processing`). `CORE_WORK_QUEUE_MAX_ACTIVE` defaults to 10000. When a new active row would exceed the bound, direct enqueue (`enqueue_core_work_for_account`) returns a backpressure error and emits `core_work_enqueue_backpressure_total`. Callers that must not drop work instead use `try_enqueue_core_work_for_account`, which returns a `Backpressured` outcome; for example, a worker that needs a conflict review leaves the `subagent_tasks` row `pending` and logs the deferral rather than failing. The coordinator then reschedules any pending sub-agent task that has no active core-work row on a later tick (`reschedule_pending_subagent_work`), so deferred reviews are picked up once pressure clears. Refreshing an existing active idempotency key is still allowed because it does not increase queue pressure. Core status includes active/max pressure so dashboards can render the queue as constrained before operators misread a saturated queue as idle.
